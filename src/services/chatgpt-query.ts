...
import { type PuppeteerLaunchOptions, Scrapeless } from '@scrapeless-ai/sdk';
...
export class ChatgptService {
  ...
  async solver(input: QueryChatgptRequest, checkTimeout: () => boolean): Promise<BaseOutput> {
    ...
    try {
      ...
      if ((error as ChatgptResponse).success) {
        const payload = handleChatResponse(error as ChatgptResponse);
        this.pushToMessage(payload, webhook);
        return createResponse(JSON.stringify(payload), (error as ChatgptResponse).url);
      }
      if ((error as ChatgptResponse).error_reason) {
        const errorMessage = (error as ChatgptResponse).error_reason;
        const payload = handleChatResponse(error as ChatgptResponse);
        this.pushToMessage(payload, webhook);
        ...
        client.on('Fetch.requestPaused', async (event) => {
          ...
          if (request.url.includes('/conversation') && isSSE) {
            try {
              ...
              console.warn('Failed to get see stream response', (err as Error)?.message);
            }
          }
  ...
  private async connectToBrowser(opt: PuppeteerLaunchOptions, checkTimeout: () => boolean) {
    ...
    try {
      ...
      this.logger.error(`Browser connection failed: ${(error as Error)?.message}`);
      throw error;
    }
  }
